...
export function validateCommand(
  ...
  if (BLOCKED_COMMANDS.has(baseCommand)) {
    logger.warn("[COMMAND_SECURITY] Blocked command attempted", {
      command: baseCommand,
    });
    return {
      isValid: false,
    ...
    logger.warn(
      "[COMMAND_SECURITY] Unknown command, treating as potentially dangerous",
      { command: baseCommand }
    );
    securityLevel = CommandSecurityLevel.APPROVAL_REQUIRED;
  }
  ...
  for (const pattern of INJECTION_PATTERNS) {
    if (pattern.test(command)) {
      ...
      const isSafeRedirection = SAFE_REDIRECTION_PATTERNS.some((safe) =>
        safe.test(command)
      );
      if (!isSafeRedirection) {
        logger.warn(
          "[COMMAND_SECURITY] Injection pattern detected in command",
          {
            command,
  ...
  for (const arg of args) {
    ...
    for (const pattern of INJECTION_PATTERNS) {
      if (pattern.test(arg)) {
        ...
        if (
          (baseCommand === "sed" || baseCommand === "awk") &&
          /[{}$]/.test(arg)
        ) {
        ...
        if (
          (baseCommand === "find" || baseCommand === "ls") &&
          /[*?[\]]/.test(arg)
        ) {
        ...
        logger.warn(
          "[COMMAND_SECURITY] Injection pattern detected in argument",
          {
            arg,
            pattern: pattern.toString(),
          }
    ...
    if (hasDangerousPattern) {
      return {
        isValid: false,
        error: "Argument contains potentially dangerous characters",
      };
    }
    ...
    if (arg.length > 1000) {
      return {
        isValid: false,
        error: "Argument too long (max 1000 characters)",
      };
    }
  ...
  if (securityLevel === CommandSecurityLevel.APPROVAL_REQUIRED) {
    logger.info("[COMMAND_SECURITY] Executing approval-required command", {
      ...
      args: sanitizedArgs,
      validationDetails:
        validationDetails.length > 0 ? validationDetails : undefined,
    });
  }
...
function validateSpecificCommand(
  ...
  switch (command) {
    ...
    case "rmdir":
      ...
      if (args.some((arg) => arg === "-rf" || arg === "-fr")) {
        if (args.some((arg) => arg === "/" || arg === "/*")) {
          return {
            isValid: false,
            error: "Cannot use rm -rf on root directory",
          };
        }
      ...
      for (const arg of args) {
        ...
        if (!isPathWithinWorkspace(arg, workspaceRoot)) {
          return {
            isValid: false,
            error: `Path '${arg}' is outside workspace`,
          };
        }
        ...
        if (importantFiles.some((f) => arg.includes(f))) {
          validationDetails?.push(
            `Warning: Removing ${arg} which appears to be an important file`
          );
        }
      }
      break;
    ...
    case "cp": {
      ...
      if (paths.length >= 2) {
        ...
        if (dest && importantFiles.some((f) => dest.includes(f))) {
          validationDetails?.push(
            `Warning: Overwriting ${dest} which appears to be an important file`
          );
        }
      }
      break;
    ...
    case "fish": {
      ...
      if (
        args.length === 0 ||
        args.some((arg) => arg === "-i" || arg === "--interactive")
      ) {
        return {
          isValid: false,
          error: "Interactive shell sessions are not allowed",
        };
      }
      ...
      if (command === "kill") {
        ...
        for (const pid of pids) {
          if (!/^\d+$/.test(pid)) {
            return {
              isValid: false,
              error: "Kill command only accepts numeric PIDs",
            };
          }
        }
      } else {
        validationDetails?.push(
          "Warning: killall/pkill can affect multiple processes"
        );
      }
      break;
    ...
    case "wget": {
      ...
      if (outputIndex !== -1 && outputIndex + 1 < args.length) {
        ...
        if (outputPath && !isPathWithinWorkspace(outputPath, workspaceRoot)) {
          return {
            isValid: false,
            error: "Output file must be within workspace",
          };
        }
      }
      break;
    ...
    case "docker":
      ...
      if (args.includes("--privileged")) {
        return {
          isValid: false,
          error: "Privileged docker containers are not allowed",
        };
      }
      if (
        args.some(
          (arg) => arg.includes("--user=root") || arg.includes("-u=root")
        )
      ) {
        return {
          isValid: false,
          error: "Running docker as root is not allowed",
        };
      }
      if (args.includes("run") && !args.includes("--rm")) {
        validationDetails?.push(
          "Consider using --rm flag to auto-remove container"
        );
      }
      break;
    ...
    case "pnpm":
      ...
      if (args.includes("-g") || args.includes("--global")) {
        validationDetails?.push(
          "Warning: Global package installation requested"
        );
      }
    ...
    case "git":
      ...
      if (args.includes("reset") && args.includes("--hard")) {
        validationDetails?.push(
          "Warning: Hard reset will lose uncommitted changes"
        );
      }
      break;
  }
