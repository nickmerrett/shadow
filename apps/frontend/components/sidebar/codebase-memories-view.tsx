...
export function SidebarCodebaseMemoriesView({ codebaseId }: { codebaseId: string }) {
  ...
  // Load memories when component mounts or codebase changes
  useEffect(() => {
    const loadMemories = async () => {
      if (!session?.user?.id || !codebase?.repoFullName) return;

      setIsLoading(true);
      try {
        const response = await fetch(
          `/api/settings/${session.user.id}/memories?repoFullName=${codebase.repoFullName}`
        );
        if (response.ok) {
    ...
    loadMemories();
  }, [session?.user?.id, codebase?.repoFullName]);
