...
const { GraphNode, GraphEdge, makeId } = require('./graph');

function chunkSymbol({repoId, fileNode, sym, lang, sourceText, maxLines=200}){
  ...
  if(len <= maxLines){
    ...
    chunks.push(new GraphNode({
      id, kind:'CHUNK', name:sym.name, path:fileNode.path, lang,
      loc:sym.loc, code
    }));
    return chunks;
  }
  ...
  for(let i=0;i<sym.node.namedChildCount;i++){
    ...
    const cLoc = {
      ...
      byteStart:c.startIndex, byteEnd:c.endIndex,
    };
  ...
  if(childBlocks.length === 0){
    ...
    while(start <= sym.loc.endLine){
      ...
      const loc = {startLine:start,startCol:0,endLine:end,endCol:0,byteStart:0,byteEnd:0};
      const code = lines.slice(start,end+1).join('\n');
      const id = makeId(repoId, fileNode.path, `CHUNK${idx}`, sym.name, loc);
      chunks.push(new GraphNode({id, kind:'CHUNK', name:`${sym.name}#${idx}`, path:fileNode.path, lang, loc, code, meta:{strategy:'sliding'}}));
      idx++;
      start = end+1;
    }
    return chunks;
  ...
  for(const m of merged){
    ...
    chunks.push(new GraphNode({id, kind:'CHUNK', name:`${sym.name}#${idx}`, path:fileNode.path, lang, loc:m.loc, code, meta:{strategy:'ast'}}));
    idx++;
  }
  return chunks;
}

module.exports = { chunkSymbol };
