...
class SetupValidator {
  ...
  private async checkNodeVersion() {
    try {
      ...
      if (majorVersion >= 18) {
        ...
        this.addResult('Node.js Version', 'fail', `${version} (requires >= 18.0.0)`);
      }
    } catch (error) {
  ...
  private async checkAPIKeys() {
    ...
    if (hasAnthropic && hasOpenAI) {
      this.addResult('API Keys', 'pass', 'Both Anthropic and OpenAI keys configured');
    } else if (hasAnthropic) {
      this.addResult('API Keys', 'pass', 'Anthropic API key configured');
    } else if (hasOpenAI) {
      this.addResult('API Keys', 'pass', 'OpenAI API key configured');
    } else {
      this.addResult('API Keys', 'fail', 'No API keys configured. Set ANTHROPIC_API_KEY or OPENAI_API_KEY in .env');
    }
  }
  ...
  private async checkDatabase() {
    try {
      ...
      this.addResult('Database', 'fail', `Connection failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async checkRipgrep() {
    try {
      ...
      this.addResult('ripgrep', 'fail', 'ripgrep not found. Install with: brew install ripgrep (macOS) or apt install ripgrep (Ubuntu)');
    }
  }
    ...
    try {
      ...
      if (stats.isDirectory()) {
        ...
        try {
          ...
          this.addResult('Workspace Directory', 'pass', `${workspaceDir} exists and is writable`);
        } catch (writeError) {
          this.addResult('Workspace Directory', 'warning', `${workspaceDir} exists but may not be writable`);
        }
      } else {
        this.addResult('Workspace Directory', 'fail', `${workspaceDir} exists but is not a directory`);
      }
    } catch (error) {
      if (error instanceof Error && 'code' in error && error.code === 'ENOENT') {
        this.addResult('Workspace Directory', 'warning', `${config.workspaceDir} does not exist. Run: mkdir -p ${config.workspaceDir}`);
      } else {
        this.addResult('Workspace Directory', 'fail', `Error accessing ${config.workspaceDir}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  }
  ...
  private async checkDependencies() {
    try {
      ...
      if (missingDeps.length === 0) {
        this.addResult('Dependencies', 'pass', 'All required packages installed');
      } else {
        this.addResult('Dependencies', 'fail', `Missing dependencies: ${missingDeps.join(', ')}`);
      }
    } catch (error) {
      this.addResult('Dependencies', 'warning', 'Could not verify dependencies');
    }
  }
