...
async function getCommittedChanges(workspacePath: string, allFiles: Map<string, FileChange>, now: string) {
...
async function getUncommittedChanges(workspacePath: string, allFiles: Map<string, FileChange>, now: string) {
  const startTime = Date.now();
  ...
  if (uncommittedOutput.trim()) {
    ...
    const uncommittedFiles: Array<{ filePath: string; status: string; operation: FileChange['operation'] }> = [];
    ...
    if (uncommittedFiles.length > 0) {
        ...
        try {
          ...
          if (status.includes('??')) {
            ...
            if (indexStatus !== ' ' && indexStatus !== '?') {
              ...
              try {
                const { stdout: stagedOutput } = await execAsync(stagedCommand, { cwd: workspacePath });
            ...
            if (workingStatus !== ' ' && workingStatus !== '?') {
              ...
              try {
                const { stdout: unstagedOutput } = await execAsync(unstagedCommand, { cwd: workspacePath });
      ...
      const diffStatsMap = new Map(
        diffResults.map(result => [result.filePath, { additions: result.additions, deletions: result.deletions }])
      );
