...
import { REQUIRE_TERMINAL_APPROVAL, approveTerminalCommand, getPendingCommands } from './tools';
...
class LocalCodingAgent {
  ...
  async start() {
    ...
    this.rl.on('line', async (input) => {
      ...
      if (line === '/clear') {
        ...
        console.log('🧹 Conversation history cleared. New task ID:', this.taskId);
        this.rl.prompt();
        return;
      }
  ...
  private showHelp() {
    ...
    console.log('==========================');
    console.log('I\'m a coding assistant that can help you with various programming tasks.');
  ...
  private async processCodingRequest(request: string) {
    try {
      ...
      await this.chatService.processCodingTask(this.taskId, request, this.model);
  ...
  private async handlePendingCommands() {
    ...
    for (const { id, command } of pending) {
      ...
      if (approveTerminalCommand(id, approved)) {
        console.log(approved ? '✅ Command approved and executed' : '❌ Command rejected');
      } else {
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--model' && i + 1 < args.length) {
      ...
      console.log('');
      console.log('Available models: claude-3-5-sonnet-20241022, claude-3-5-haiku-20241022, gpt-4o, gpt-4o-mini');
      process.exit(0);
    }
  }
